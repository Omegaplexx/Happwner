 ...
 *
 * Most Xray protocols accept both a nested server list (`vnext`/`servers`) and
 * a flat single-server form; both are read here, and each server/user pair
 * becomes one mihomo proxy.
 */
...
/**
 * Trims the flow to the part mihomo understands. Xray's "-udp443" suffix only
 * changes how UDP/443 is handled, and mihomo drops it.
 */
...
/**
 * Validates Xray's VLESS encryption string and returns it in the form mihomo
 * expects, which is the same string.
 *
...
/**
 * Checks the padding descriptors of a VLESS encryption string. Each one is
 * "a-b-c"; they alternate between length and gap descriptors, and the first one
 * has minimum values.
 *
...
internal fun MihomoConv.convertShadowsocks(ob: JSONObject, preferred: String): List<YamlMap> {
    ...
    for (sv in servers) {
        ...
        m.set("cipher", cipher)
        m.set("password", sv.xStr("password"))
        udpFlag(m)
        if (sv.xBool("uot")) {
...
/** Normalises a shadowsocks method name to mihomo's spelling. */
internal fun ssCipher(method: String): String = when (val m = method.trim().lowercase()) {
    ...
    // AEAD, AEAD-2022 and the stream ciphers use the same names in both projects.
    else -> m
}
...
private fun proxyEndpoints(s: JSONObject): List<ProxyEndpoint> {
    ...
    for (sv in s.xObjList("servers")) {
        ...
        if (users.isNotEmpty()) {
            ...
            pass = users[0].xStrOf("pass", "password")
        }
        out.add(
            ProxyEndpoint(
