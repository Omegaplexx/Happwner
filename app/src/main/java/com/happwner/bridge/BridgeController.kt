...
object BridgeController {
    ...
    const val ACTION_TOGGLE = ModuleIds.ACTION_TOGGLE_BRIDGE
    ...
    fun notifyChanged(context: Context) {
        val app = context.applicationContext
        try {
            app.sendBroadcast(Intent(ModuleIds.ACTION_REFRESH_UI).setPackage(app.packageName))
        } catch (_: Throwable) {
        }
        refreshSurfaces(app)
    }
    ...
    fun localizedContext(context: Context): Context = AppLocale.wrap(context)
}
