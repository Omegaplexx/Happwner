...
// The mihomo-specific Xray readers: ranges, header maps, transport-name detection, and the
// base64/hex codecs the link parsers need.

// Xray's Int32Range: a plain number or a "from-to" string. Mihomo takes the same information as a
// string, so render gives it back in that form.
...
// Reads Xray's port list in any of its spellings: a number, a "1000-2000" range, a comma-separated
// list, or a JSON array of those. The textual form is what mihomo's "ports" takes.
...
internal fun streamNetwork(ss: JSONObject?): String {
    ...
    return when (n) {
        ...
        "mkcp" -> "kcp"
        "gun" -> "grpc"
        // In Xray, "http" is the HTTP/2 transport. It is not the same thing as mihomo's "http"
        // network, which is the HTTP masquerade that Xray spells as a raw-TCP header.
        "http", "h2", "h3" -> "h2"
        else -> n
    }
}
...
internal fun streamSecurity(ss: JSONObject?): String {
    ...
    return when (val sec = ss.xStr("security").lowercase()) {
        "", "none", "auto" -> {
            // A config may omit "security" but still carry tlsSettings; Xray ignores that, and so
            // do we, except for REALITY which is only ever present deliberately.
...
// Applies XHTTP's "extra" object the way Xray does: everything comes from "extra" except host, path
// and mode.
...
// Removes `//` and slash-star comments and trailing commas: Xray accepts commented JSON and
// hand-maintained configs use it heavily. Comment characters inside string literals are left alone.
...
// Removes commas that directly precede a closing brace or bracket, which JSON rejects but hand-
// written configs often contain.
...
// Decodes base64 with the given alphabet. padded mirrors how Go tells the padded and raw encodings
// apart: true requires the "=" padding, false rejects it.
