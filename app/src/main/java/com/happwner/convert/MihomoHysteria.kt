...
// A Hysteria2 node arrives in two shapes.

// Gathers a Hysteria2 node's settings from wherever the config puts them. Values in the outbound's
// own settings win - a generator that filled those meant them.
internal class HysteriaParams(
    var password: String = "",
    var up: String = "",
    var down: String = "",
    var ports: String = "",
...
// Reads the parts of a Hysteria2 node Xray keeps in streamSettings, where a natively written config
// puts the password. Drops go into notes so the sing-box converter can share this reader.
internal fun HysteriaParams.fromStream(notes: MutableList<String>, ss: JSONObject?) {
    if (ss == null) return

    val h = ss.xObj("hysteriaSettings")
    if (h != null) {
        password = firstNonEmpty(password, h.xStrRaw("auth"))
        ...
        if (h.xInt("udpIdleTimeout") != 0) {
            notes.add("hysteriaSettings.udpIdleTimeout has no mihomo equivalent and was dropped")
        }
    }
    ...
    if (mask != null && obfs.isEmpty()) {
        ...
        obfsPassword = firstNonEmpty(obfsPassword, settings.xStrRaw("password"))
...
internal class HysteriaEndpoint(
    val address: String,
    val port: Int,
    val ports: String,
    val password: String,
...
// Resolves the server list of a Hysteria settings object, falling back to the flat form.
internal fun hysteriaEndpoints(s: JSONObject): List<HysteriaEndpoint> {
    val flatPassword = firstNonEmpty(
        s.xStrRaw("password"), s.xStrRaw("auth"), s.xStrRaw("auth_str"), s.xStrRaw("authStr")
    )
        ...
        out.add(
            HysteriaEndpoint(
                ...
                password = firstNonEmpty(
                    sv.xStrRaw("password"), sv.xStrRaw("auth"), sv.xStrRaw("auth_str"), flatPassword
                ),
                // up_mbps/down_mbps is what the Hysteria v1 client itself writes, so a node copied
                // out of one used to arrive with no rates at all and be dropped for lacking them.
                up = firstNonEmpty(
                    sv.xStr("up"), mbpsString(sv.xInt("upMbps")), mbpsString(sv.xInt("up_mbps")), flatUp
                ),
                down = firstNonEmpty(
                    sv.xStr("down"), mbpsString(sv.xInt("downMbps")), mbpsString(sv.xInt("down_mbps")), flatDown
                )
            )
        )
    }
...
// Resolves the up/down rates from any of the spellings the Hysteria clients accept.
internal fun hysteriaBandwidth(s: JSONObject): Pair<String, String> {
    var up = s.xStr("up")
    ...
    up = firstNonEmpty(up, mbpsString(s.xInt("upMbps")), mbpsString(s.xInt("up_mbps")))
    down = firstNonEmpty(down, mbpsString(s.xInt("downMbps")), mbpsString(s.xInt("down_mbps")))
    return up to down
}

internal fun mbpsString(v: Int): String = if (v <= 0) "" else "$v Mbps"

// Renders a rate the way mihomo expects. A bare number is megabits per second there, so a unit-less
// value gets the unit spelled out.
...
// The obfuscation of a hysteria node, wherever it was written: beside the protocol or beside the
// address. A node handed out without it is one the server will not answer.
internal fun parsedObfsAnywhere(s: JSONObject): Pair<String, String> {
    val here = parsedObfs(s)
    if (here.first.isNotEmpty() || here.second.isNotEmpty()) return here
    val server = s.xObjList("servers").firstOrNull() ?: return here
    return parsedObfs(server)
}

// Reads the obfuscation of one object, accepting both the string form ("salamander") and the
// object form used by the Hysteria2 client.
internal fun parsedObfs(s: JSONObject): Pair<String, String> {
    ...
    if (obfs is JSONObject) {
        ...
        if (salamander != null && salamander.xStrRaw("password").isNotEmpty()) {
            password = salamander.xStrRaw("password")
        } else if (obfs.xStrRaw("password").isNotEmpty()) {
            password = obfs.xStrRaw("password")
        }
...
internal fun hysteriaHopInterval(s: JSONObject): String {
...
internal class HysteriaTls(val sni: String, val insecure: Boolean, val alpn: List<String>)

// Resolves the TLS parameters, which may live in the settings object or in streamSettings depending
// on which tool wrote the config.
internal fun hysteriaTls(s: JSONObject, ss: JSONObject?): HysteriaTls {
    var sni = ""
    var insecure = false
...
internal fun firstPortOfRange(ports: String): Int? {
...
internal fun MihomoConv.convertHysteria2(ob: JSONObject, preferred: String): List<YamlMap> {
    ...
    for (ep in hysteriaEndpoints(settings)) {
        ...
        val streamNotes = ArrayList<String>()
        params.fromStream(streamNotes, ss)
        for (n in streamNotes) warn(n)
        ...
        // Hysteria spells its own pin "pinSHA256"; the shared tlsSettings one
        // below has three spellings of its own, which applyCertPin sorts out.
        settings.xObj("tls")?.let {
            applyCertPinValue(m, it.xStr("pinSHA256"), "hysteria2Settings.tls.pinSHA256")
        }
        val t = streamTls(ss)
        if (t != null) {
            applyCertPin(m, t)
...
// Whether a "hysteria" outbound is version 2.
internal fun isHysteria2(s: JSONObject, ss: JSONObject?): Boolean {
    ...
    if (endpoints.isEmpty()) {
        endpoints.add(
            TuicEndpoint(
                ...
                password = settings.xStrRaw("password"),
