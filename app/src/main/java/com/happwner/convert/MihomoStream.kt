...
// Describes how a protocol spells its TLS fields, which differs between the V2Ray-family protocols
// and the rest.
internal class TlsTarget(
    // "servername" for vless/vmess, "sni" for everything else.
    val sniKey: String,
    // True when the protocol needs "tls: true" to enable TLS. Trojan, Hysteria2 and TUIC are always
    // TLS and have no such field.
    val explicitTls: Boolean
)

// Deliberately no supportsReality/supportsEch flags: measured against mihomo v1.19.29 (by feeding
// it deliberately broken values and seeing whether it objected), the core parses REALITY and ECH
internal val TLS_V2RAY = TlsTarget("servername", explicitTls = true)
internal val TLS_TROJAN = TlsTarget("sni", explicitTls = false)
...
internal fun MihomoConv.applyTls(m: YamlMap, ss: JSONObject?, target: TlsTarget) {
    ...
    when (security) {
        ...
        // REALITY is a TLS variant, so both switch the same field on.
        "tls", "reality" -> if (target.explicitTls) m.set("tls", true)
        else -> {
    ...
    applyNameCertVerify(m, tlsCfg)
    applyClientFingerprint(m, tlsCfg.xStr("fingerprint"))
    applyCertPin(m, tlsCfg)
    ...
    if (tlsCfg.xStr("cipherSuites").isNotEmpty()) {
        warn("tlsSettings.cipherSuites has no mihomo equivalent and was dropped")
    }
    if (tlsCfg.xBool("disableSystemRoot")) {
        warn("tlsSettings.disableSystemRoot has no mihomo equivalent; the system roots stay in use")
    }
    if (tlsCfg.xStr("masterKeyLog").isNotEmpty()) {
        warn("tlsSettings.masterKeyLog has no mihomo equivalent and was dropped")
    }
}
...
private fun MihomoConv.applyReality(m: YamlMap, ss: JSONObject?, target: TlsTarget) {
    ...
    if (r.xStr("spiderX").isNotEmpty()) {
        warn("realitySettings.spiderX has no mihomo equivalent and was dropped")
    }
...
// The REALITY public key. Newer Xray builds call the client-side field "password"; older ones call
// it "publicKey".
internal fun realityPublicKey(r: JSONObject): String = r.xStrOf("publicKey", "password")

// The uTLS profiles mihomo resolves to a real ClientHello, measured against v1.19.29: the core accepts
// any string and silently falls back to Go's own crypto/tls, which is trivially fingerprintable.
private val UTLS_FINGERPRINTS = setOf(
    "chrome", "firefox", "safari", "ios", "android", "edge",
    "360", "qq", "random", "randomized",
    "chrome120", "firefox120"
)

// Names with an exact counterpart under a different spelling.
private val UTLS_ALIASES = mapOf(
    "randomizedalpn" to "random",
    "randomizednoalpn" to "random"
)

// Families a versioned uTLS name can be folded onto.
private val UTLS_FAMILIES = listOf("chrome", "firefox", "safari", "ios", "android", "edge")

// Maps a versioned uTLS profile onto its family, which the core does resolve; the next character must
// not be a letter, so "chromium" isn't read as "chrome". Measured: the rest fall back to plain Go TLS.
private fun utlsFamilyOf(fp: String): String? = UTLS_FAMILIES.firstOrNull {
    fp.length > it.length && fp.startsWith(it) && !fp[it.length].isLetter()
}
...
internal fun MihomoConv.applyClientFingerprint(m: YamlMap, raw: String) {
    ...
    // The core's lookup is exact and case-sensitive: "chrome_120", "chrome-120" and "Chrome120" all
    // fall through to a plain Go handshake.
    val compact = fp.filterNot { it == '_' || it == '-' || it == '.' || it == ' ' }
    if (compact != fp && compact in UTLS_FINGERPRINTS) {
        m.set("client-fingerprint", compact)
        warn("uTLS fingerprint \"$fp\" is spelled \"$compact\" in mihomo; used that")
        return
    }

    UTLS_ALIASES[fp]?.let {
        m.set("client-fingerprint", it)
        warn("uTLS fingerprint \"$fp\" is not one mihomo resolves; used \"$it\"")
        return
    }
    utlsFamilyOf(fp)?.let {
        m.set("client-fingerprint", it)
        // Not "unknown to mihomo": the core takes the name without complaint and then quietly sends
        // a plain Go ClientHello.
        warn(
            "uTLS fingerprint \"$fp\" is not one mihomo resolves; it would have " +
                "silently disabled uTLS, so \"$it\" was used instead"
        )
        return
    }
    when (fp) {
        "unsafe", "golang", "hellogolang" ->
            ...
            warn("unknown uTLS fingerprint \"$fp\" was dropped; mihomo will use a plain Go TLS handshake")
    }
}

// Carries Xray's certificate pin to mihomo's "fingerprint", reading both spellings. The public-key
// variant is deliberately not mapped: it hashes a different preimage, so the pin could never match.
internal fun MihomoConv.applyCertPin(m: YamlMap, tlsCfg: JSONObject) {
    if (tlsCfg.xStrList("pinnedPeerCertificatePublicKeySha256").isNotEmpty()) {
        warn(
            "tlsSettings.pinnedPeerCertificatePublicKeySha256 pins the public key, " +
                "which mihomo cannot express - its \"fingerprint\" pins the certificate - so it was dropped"
        )
    }

    // The scalar spelling first, then the list one, so a configuration carrying both uses what the
    // newer core would.
    val candidates = listOf(
        "pinnedPeerCertSha256" to tlsCfg.xStrList("pinnedPeerCertSha256"),
        "pinnedPeerCertificateChainSha256" to tlsCfg.xStrList("pinnedPeerCertificateChainSha256")
    )
    val (field, pins) = candidates.firstOrNull { it.second.isNotEmpty() } ?: return

    if (pins.size > 1) {
        warn("tlsSettings.$field lists ${pins.size} pins; mihomo takes one, so the first was used")
    }
    applyCertPinValue(m, pins.first(), "tlsSettings.$field")
}

// Emits one SHA-256 certificate pin, accepting Xray's base64 form or a hex one. field names the
// source for the diagnostic, since the same value arrives under several spellings.
internal fun MihomoConv.applyCertPinValue(m: YamlMap, raw: String, field: String) {
    val pin = raw.trim()
    ...
    warn("could not decode $field as a SHA-256 value; it was dropped")
}
...
// Xray's verifyPeerCertInNames checks the peer certificate against these names instead of the SNI,
// which is what mihomo's name-cert-verify does; mihomo takes one name, so the first is used.
internal fun MihomoConv.applyNameCertVerify(m: YamlMap, tlsCfg: JSONObject) {
    val names = tlsCfg.xStrList("verifyPeerCertInNames")
    if (names.isEmpty()) return
    m.setStr("name-cert-verify", names.first())
    if (names.size > 1) {
        warn(
            "tlsSettings.verifyPeerCertInNames lists ${names.size} names; mihomo verifies against " +
                "one, so the first was used"
        )
    }
}

// Maps Xray's ECH configuration list onto mihomo's ech-opts.
internal fun MihomoConv.applyEch(m: YamlMap, tlsCfg: JSONObject) {
...
// Handles the raw transport, including its HTTP masquerade, which mihomo exposes as network "http".
...
// Handles both the websocket transport and HTTPUpgrade, which mihomo models as websocket with a
// flag.
private fun MihomoConv.applyWebsocket(m: YamlMap, ss: JSONObject?, httpUpgrade: Boolean) {
    m.set("network", "ws")
    // Xray keeps a websocket alive with a periodic ping and mihomo has no setting for one, so a
    // node on a network that drops idle connections would drop them again unexplained.
    if (streamWs(ss).xInt("heartbeatPeriod") > 0) {
        warn("wsSettings.heartbeatPeriod has no mihomo equivalent and was dropped")
    }
    val optsMap = YamlMap()
...
private fun MihomoConv.applyGrpc(m: YamlMap, ss: JSONObject?) {
    ...
    if (g != null) {
        ...
        if (g.xBool("multiMode")) {
            warn("the node still uses gRPC, but grpcSettings.multiMode has no mihomo equivalent; the node will use single mode")
        }
        if (g.xStr("authority").isNotEmpty()) {
            warn("the node still uses gRPC, but grpcSettings.authority has no mihomo equivalent and was dropped")
        }
        if (g.xInt("initial_windows_size") > 0) {
            warn(
                "the node still uses gRPC, but grpcSettings.initial_windows_size has no " +
                    "mihomo equivalent and was dropped"
            )
        }
    }
...
private fun MihomoConv.applyKcp(m: YamlMap, ss: JSONObject?) {
    ...
    if (k != null) {
        ...
        // Xray counts these buffers in megabytes and mihomo in bytes, which is a conversion rather
        // than a missing feature: mihomo's own 2097152 default is Xray's 2 MB written out.
        kcpBufferBytes(k, "readBufferSize")?.let { optsMap.set("read-buffer", it) }
        kcpBufferBytes(k, "writeBufferSize")?.let { optsMap.set("write-buffer", it) }
    }
    m.set("mkcp-opts", optsMap)
}

// One mKCP buffer in the bytes mihomo counts, or null when the source did not set it. Read as a
// Long because a large megabyte figure would overflow an Int once multiplied out.
private fun kcpBufferBytes(k: JSONObject, field: String): Long? {
    val mb = k.xLong(field)
    if (mb <= 0L || mb > MAX_KCP_BUFFER_MB) return null
    return mb * 1024L * 1024L
}

// Well past any real buffer, and far enough below the point where the multiplication above would
// wrap into a negative size - which mihomo would take as a setting rather than as nonsense.
private const val MAX_KCP_BUFFER_MB = 64L * 1024L

// Maps Xray's XHTTP transport, including the "extra" object, xmux and downloadSettings, onto
// mihomo's xhttp-opts.
...
// Maps an XHTTP downloadSettings block, which is a full stream configuration plus its own address.
    ...
    when (streamSecurity(dl)) {
        "tls" -> {
            ...
            if (t != null) {
                ...
                applyNameCertVerify(out, t)
                applyClientFingerprint(out, t.xStr("fingerprint"))
            }
        }
        "reality" -> {
            out.set("tls", true)
            ...
            if (r != null) {
                ...
                applyClientFingerprint(out, r.xStr("fingerprint"))
            }
        }
    }
    return out
...
// Pulls Xray's "ed" query parameter out of a transport path, returning the path without it and the
// early-data size. Remaining parameters keep their original text and order.
