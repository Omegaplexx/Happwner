package com.happwner.crypto

import android.util.Base64
import android.util.Log
import com.happwner.convert.JsonDepth
import com.happwner.convert.toHex
import java.nio.ByteBuffer
import java.nio.charset.CharacterCodingException
import java.nio.charset.CodingErrorAction
import java.security.MessageDigest
import javax.crypto.Cipher
import javax.crypto.spec.GCMParameterSpec
import javax.crypto.spec.SecretKeySpec
import org.json.JSONObject

// Decryptor for INCY encrypted deep links (incy://crypt1/<base64url>): AES-256-GCM.
object IncyCrypto {
    private const val TAG = "Happwner:Incy"

    // Any incy://cryptN link; only crypt1 is implemented so far
    private const val CRYPT_SCHEME_PREFIX = "incy://crypt"
    private const val CRYPT1_PREFIX = "incy://crypt1/"

    // Wire format: iv(12) || ciphertext || gcmTag(16), base64url, padding stripped
    private const val IV_LEN = 12
    private const val TAG_LEN = 16
    private const val GCM_TAG_BITS = TAG_LEN * 8

    // K1 = SHA-256("incy" + "deep" + "crypt1" + "v2026.06" + keymatA[1024..1056] + keymatB[2048..2080])
    private const val SALT_P1 = "incy"
    private const val SALT_P2 = "deep"
    private const val SALT_P3 = "crypt1"
    private const val SALT_P4 = "v2026.06"
    private const val KEYMAT_A_OFFSET = 1024
    private const val KEYMAT_B_OFFSET = 2048
    private const val KEYMAT_LEN = 32

    // SHA-256(K1): confirms the bundled keymat still matches the published INCY clients
    private const val EXPECTED_KEY_FINGERPRINT =
        "b6bf708471cc90043232967660aade86a50b4e57929db2e53c5fa34db624c08c"

    sealed class Result {
        data class Decrypted(val plaintext: String, val name: String?) : Result()
        object NotCryptLink : Result()
        data class Error(val reason: String) : Result()
    }

    // Is this an incy://cryptN link?
    fun isCryptLink(link: String?): Boolean {
        if (link == null) return false
        val t = link.trim()
        return t.regionMatches(0, CRYPT_SCHEME_PREFIX, 0, CRYPT_SCHEME_PREFIX.length, ignoreCase = true)
    }

    // Decrypt an incy://crypt1/ link back to its subscription URL
    fun decryptCryptLink(link: String?): Result {
        if (link.isNullOrEmpty()) return Result.NotCryptLink
        val trimmed = link.trim()
        if (!isCryptLink(trimmed)) return Result.NotCryptLink
        if (!trimmed.regionMatches(0, CRYPT1_PREFIX, 0, CRYPT1_PREFIX.length, ignoreCase = true)) {
            // crypt2 and later would use a different key; fail loudly instead of silently
            return Result.Error("unsupported INCY scheme version (only crypt1 is implemented)")
        }

        val payload = trimmed.substring(CRYPT1_PREFIX.length).trimEnd('/').trim()
        if (payload.isEmpty()) return Result.Error("empty payload")

        val wire = try {
            b64UrlDecodeFlexible(payload)
        } catch (_: IllegalArgumentException) {
            return Result.Error("payload is not valid base64url")
        }
        if (wire.size < IV_LEN + TAG_LEN + 1) {
            return Result.Error("payload too short: ${wire.size}B, need more than ${IV_LEN + TAG_LEN}B")
        }

        val key = try {
            deriveKey()
        } catch (e: Exception) {
            val reason = e.message ?: "key derivation failed"
            Log.w(TAG, "Key derivation failed: $reason")
            return Result.Error(reason)
        }

        val plain = try {
            val cipher = Cipher.getInstance("AES/GCM/NoPadding")
            cipher.init(Cipher.DECRYPT_MODE, key, GCMParameterSpec(GCM_TAG_BITS, wire, 0, IV_LEN))
            // JCE expects ciphertext||tag, which is exactly the rest of the wire buffer
            cipher.doFinal(wire, IV_LEN, wire.size - IV_LEN)
        } catch (_: Exception) {
            return Result.Error("authentication failed (wrong key or tampered payload)")
        }

        val json = decodeStrictUtf8(plain) ?: return Result.Error("plaintext is not valid UTF-8")

        // The GCM tag proves the payload was made with the bundled key - but that key ships inside
        // the APK, so anyone can extract it and mint one.
        if (JsonDepth.exceedsMaxDepth(json)) return Result.Error("malformed plaintext")

        val obj = try {
            JSONObject(json)
        } catch (_: Exception) {
            return Result.Error("malformed plaintext")
        }

        val url = obj.optString("url").trim()
        if (url.isEmpty()) return Result.Error("missing url field")
        val name = obj.optString("n").trim().ifEmpty { null }

        Log.d(TAG, "Decrypted crypt1: ${wire.size}B wire -> ${plain.size}B payload, name=${name != null}")
        return Result.Decrypted(url, name)
    }

    private val keyLock = Any()

    @Volatile
    private var keyCache: SecretKeySpec? = null

    // Rebuild K1 from the bundled keymat: slice both blobs at their fixed offsets,
    // prepend the four salt parts, SHA-256 the lot, then verify the fingerprint.
    private fun deriveKey(): SecretKeySpec {
        keyCache?.let { return it }
        synchronized(keyLock) {
            keyCache?.let { return it }

            val a = Base64.decode(KEYMAT_A_B64, Base64.DEFAULT)
            val b = Base64.decode(KEYMAT_B_B64, Base64.DEFAULT)
            if (a.size < KEYMAT_A_OFFSET + KEYMAT_LEN || b.size < KEYMAT_B_OFFSET + KEYMAT_LEN) {
                throw IllegalStateException("bundled keymat is smaller than expected")
            }

            val seed = MessageDigest.getInstance("SHA-256")
            seed.update(SALT_P1.toByteArray(Charsets.UTF_8))
            seed.update(SALT_P2.toByteArray(Charsets.UTF_8))
            seed.update(SALT_P3.toByteArray(Charsets.UTF_8))
            seed.update(SALT_P4.toByteArray(Charsets.UTF_8))
            seed.update(a, KEYMAT_A_OFFSET, KEYMAT_LEN)
            seed.update(b, KEYMAT_B_OFFSET, KEYMAT_LEN)
            val k1 = seed.digest()

            val fingerprint = toHex(MessageDigest.getInstance("SHA-256").digest(k1))
            if (fingerprint != EXPECTED_KEY_FINGERPRINT) {
                throw IllegalStateException(
                    "derived K1 fingerprint mismatch (expected $EXPECTED_KEY_FINGERPRINT, got $fingerprint) - bundled keymat is out of sync with the INCY clients"
                )
            }

            val spec = SecretKeySpec(k1, "AES")
            keyCache = spec
            return spec
        }
    }

    private val HEX = "0123456789abcdef".toCharArray()

    private fun toHex(b: ByteArray): String {
        val sb = StringBuilder(b.size * 2)
        for (x in b) {
            val v = x.toInt() and 0xff
            sb.append(HEX[v ushr 4]).append(HEX[v and 0x0f])
        }
        return sb.toString()
    }

    // base64url with auto-normalization (url-safe to standard, strip whitespace, re-pad with =)
    private fun b64UrlDecodeFlexible(s: String): ByteArray {
        val sb = StringBuilder(s.length)
        for (c in s) {
            when (c) {
                ' ', '\n', '\r', '\t' -> {}
                '-' -> sb.append('+')
                '_' -> sb.append('/')
                else -> sb.append(c)
            }
        }
        val rem = sb.length % 4
        if (rem != 0) repeat(4 - rem) { sb.append('=') }
        return Base64.decode(sb.toString(), Base64.DEFAULT)
    }

    // Strict UTF-8 decode; null if not well-formed
    private fun decodeStrictUtf8(b: ByteArray): String? {
        if (b.isEmpty()) return null
        return try {
            val decoder = Charsets.UTF_8.newDecoder()
                .onMalformedInput(CodingErrorAction.REPORT)
                .onUnmappableCharacter(CodingErrorAction.REPORT)
            decoder.decode(ByteBuffer.wrap(b)).toString()
        } catch (_: CharacterCodingException) {
            null
        }
    }

    // Keymat blobs shipped with the INCY clients (4096B each). Neither one is the key:
    // only 32 bytes at a fixed offset in each feeds the SHA-256 in deriveKey().
    private const val KEYMAT_A_B64 =
        ""

    private const val KEYMAT_B_B64 =
        ""
}
